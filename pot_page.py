...
POT_CSS = """
<style>
.pot-img-placeholder{width:100%;height:160px;border-radius:14px;
  background:linear-gradient(135deg,#EEF4FF,#DBEAFE);
  display:flex;align-items:center;justify-content:center;
  font-size:3rem;margin-bottom:1rem;}
.pot-card{border:1.5px solid #E5E7EB;border-radius:14px;
...
.item-price-badge{display:inline-block;background:#EFF6FF;color:#3B82F6;
...
def _map_view_html(lat, lng, name):
    return f"""<!DOCTYPE html><html><head>
<meta charset="utf-8">
<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css"/>
<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
...
""")
    ...
    components.html(_map_view_html(pot["location_lat"], pot["location_lng"],
                                   pot["location_name"]), height=280, scrolling=False)
        ...
        with c1:
            if st.button("✅ 팟 종료", key=f"end_{pot_id}", use_container_width=True):
                ...
                push_all(pot["members"], "pot_ended",
                         f"'{pot['item_label']}' 팟이 종료됐습니다. 평가를 진행해 주세요.")
                st.success("팟이 종료됐습니다."); st.rerun()
        with c2:
    ...
    if status == "ended" and uid in pot.get("members", []) and uid not in pot.get("rated_by", []):
        ...
        if not others:
            ...
            with st.form(f"rate_form_{pot_id}"):
                for mid in others:
                    scores[mid] = st.slider(f"@{mid}", 1, 10, 7, key=f"sc_{pot_id}_{mid}")
                if st.form_submit_button("평가 제출", use_container_width=True):
                    for mid, score in scores.items():
                        add_score(mid, score)
                        push(mid, "pot_ended",
                             f"'{pot['item_label']}' 팟 평가가 완료됐습니다. 매너 온도를 확인하세요!")
                    mark_rated(pot_id, uid)
                    st.success("평가가 제출됐습니다!"); st.rerun()
...
def render_item_popup(item: dict):
    ...
    st.markdown(f"### {label}")
    if item_price > 0:
        st.markdown(f'<div class="item-price-badge">💰 정가 {item_price:,}원</div>',
    ...
    # ── CREATE ────────────────────────────────────────────────────────────
        ...
        if item_price > 0:
            ...
            price_total = st.number_input("총 가격 (원)", min_value=0,
                                          step=1000, value=10000, key="price_input")

        total_people = st.number_input("몇 명이서 나눌까요?", min_value=2,
                                       max_value=20, value=2, step=1, key="people_input")
        ppp = int(price_total) // max(int(total_people), 1)
        st.markdown(f"**👉 인당 가격: {ppp:,}원**")

        buy_date = st.date_input("구매 날짜", key="date_input")
        contact  = st.text_input("📞 대표 연락처",
                                  placeholder="카카오톡 ID, 전화번호 등", key="contact_input")

        st.markdown("**📍 수령 장소**")
        loc_name = st.text_input("장소 이름", placeholder="예: 강남역 2번 출구 앞", key="loc_name")
        cl, cr   = st.columns(2)
        with cl: loc_lat = st.number_input("위도",  value=37.5665, format="%.5f", key="loc_lat")
        with cr: loc_lng = st.number_input("경도", value=126.9780, format="%.5f", key="loc_lng")
        ...
        window.addEventListener('message',function(e){
          if(e.data&&e.data.type==='MAP_PICK'){
            var el=document.getElementById('map_coord_out');
            if(el) el.innerText='📍 위도: '+e.data.lat+' / 경도: '+e.data.lng;
          }
        });
        </script>
        <pre id="map_coord_out"
        ...
        if not pots:
            ...
            for pot in pots:
                ...
                if already_in:
                    ...
                    st.button("마감됨", key=f"full_{pot_id}",
