...
from pot_store          import (get_active_pots_by_user, get_all_pots,
                                admin_delete_pot, calc_dday, price_per_person)
...
.profile-card{background:#fff;border-radius:20px;padding:2rem 2.4rem;
  margin-bottom:1.2rem;box-shadow:0 4px 24px rgba(0,0,0,.06);
  border:1px solid #E5E7EB;display:flex;align-items:center;gap:1.6rem;}
...
.temp-bar-wrap{background:#F3F4F6;border-radius:20px;height:12px;
...
.my-pot-card{border:1.5px solid #DBEAFE;border-radius:14px;
  padding:.9rem 1.1rem;margin-bottom:.7rem;background:#F0F7FF;position:relative;}
.my-pot-card:hover{border-color:#3B82F6;}
.my-pot-dday{position:absolute;top:.65rem;right:.9rem;font-size:.68rem;
  font-weight:800;color:#EF4444;background:#FEE2E2;
...
def _general_card_html(items):
    ...
    for item in items:
        ...
        bg    = (f"url('{img}') center/cover no-repeat"
                 if img else "linear-gradient(135deg,#EEF4FF,#DBEAFE)")
...
.card{{flex:0 0 150px;height:170px;border-radius:12px;border:2px solid #3B82F6;
  ...
  justify-content:flex-end;padding-bottom:.7rem;position:relative;overflow:hidden;}}
    ...
    if step == "choice":
        ...
        <div style="text-align:center;margin-top:2.5rem;">
          <p style="font-family:'DM Serif Display',serif;font-size:1.8rem;
             color:#1a1a1a;margin-bottom:.3rem">My Page</p>
          <p style="font-size:.85rem;color:#9CA3AF;">
             로그인하거나 새 계정을 만들어 보세요</p>
...
def render_my_page():
    from pot_page import render_pot_detail  # 순환 방지 지연 import
    ...
    # ── 관리자: 전체 팟 내역 ──────────────────────────────────────────────
    if is_admin:
        st.markdown('<div class="mypage-section-title">🛡️ 전체 팟 내역</div>',
                    unsafe_allow_html=True)
    ...
    # ── 일반 유저: active 팟만 표시 ──────────────────────────────────────
    st.markdown('<div class="mypage-section-title">🛒 내 구메팟</div>',
                unsafe_allow_html=True)
    # active 팟만 (종료·해산은 기록에만 보존, 표시 안 함)
    my_pots = get_active_pots_by_user(uid)

    if not my_pots:
        st.info("진행 중인 구메팟이 없습니다.")
    else:
        for pot in my_pots:
            dday  = calc_dday(pot["buy_date"])
            ppp   = price_per_person(pot)
            ...
            <div class="my-pot-card">
              ...
              <div class="my-pot-meta">
                👥 {mem}/{total}명 · 📅 {pot['buy_date']} · 📍 {pot['location_name']}
              </div>
              <div class="my-pot-price">인당 {ppp:,}원</div>
            </div>""", unsafe_allow_html=True)

            if st.button("상세 보기", key=f"view_pot_{pot['pot_id']}"):
                st.session_state.viewing_pot_id = pot["pot_id"]; st.rerun()
    ...
    st.markdown(f'<div class="mypage-section-title">{title2}</div>',
                unsafe_allow_html=True)
